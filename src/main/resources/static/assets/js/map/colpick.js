...
(function ($) {
	var colpick = function () {
		var
			...
			//Function called when the new color is changed
			change = function (ev) {
			...
			//Change style on blur and focus of inputs
			blur = function (ev) {
			...
			//Increment/decrement arrows next to the fields
			...
			downSelector = function (ev) {
				...
				//Get colpick and position of the selector
				var current = {
					cal: $(this).parent(),
					pos: $(this).offset()
				};
			...
			init: function (opt) {
				...
				return this.each(function () {
					...
					if (!$(this).data('colpickId')) {
						...
						//Add class if the color scheme is not dark(default)
						...
						cal.find('span').mousedown(downIncrement).end().find('>div.colpick_current_color').click(restoreOriginal);
						//Setup hue selector
						options.selector = cal.find('div.colpick_color').mousedown(downSelector);
						options.selectorIndic = options.selector.find('div div');
						//Store parts of the plugin
						options.el = this;
						options.hue = cal.find('div.colpick_hue div');
						...
						//Append to the body if flat=false, else show in place
						if (options.flat) {
